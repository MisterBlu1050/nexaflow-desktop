...
export type NexaCommandResponse = {
  ...
  attachments?: Array<{ label: string; fileName?: string }>;
...
export const randTypingDelay = (): number => Math.floor(Math.random() * (1400 - 800 + 1)) + 800;
export const isoNow = (): string => new Date().toISOString();

const makeFooter = (legalRef?: string) => ({ validation: 'auto-validated', legalRef, generatedIn: isoNow() });

// /comp-analysis
export const compAnalysis = (_args?: string): NexaCommandResponse => {
...
// /draft-offer
export const draftOffer = (args?: string): NexaCommandResponse => {
  if (!args || !args.trim()) {
    return {
      command: '/draft-offer',
...
// /onboarding
export const onboarding = (_args?: string): NexaCommandResponse => {
  const preArrival = [
...
// /people-report
export const peopleReport = (_args?: string, ctx?: any): NexaCommandResponse => {
  ...
  const byDept = employees.length ? employees.reduce((acc: any, e: any) => { acc[e.department] = (acc[e.department] || 0) + 1; return acc; }, {}) : { Engineering: 120, Sales: 95, HR: 15, Product: 40 };
  const sites = employees.length ? employees.reduce((acc: any, e: any) => { acc[e.site] = (acc[e.site] || 0) + 1; return acc; }, {}) : { Brussels: 180, Amsterdam: 60, Berlin: 30 };
  const attrition = employees.length ? ( (employees.filter((e:any)=> e.status==='left').length / employees.length) * 100 ).toFixed(1) + '%' : '8.2%';
  const diversity = employees.length ? { female: `${(employees.filter((e:any)=> e.gender==='F').length)} (${Math.round((employees.filter((e:any)=> e.gender==='F').length/employees.length)*100)}%)` } : { female: '42 (34%)', underrepresented: '12%' };
...
// /performance-review
export const performanceReview = (_args?: string): NexaCommandResponse => {
...
// /policy-lookup
export const policyLookup = (args?: string): NexaCommandResponse => {
  const input = (args || '').trim().toUpperCase();
  let jurisdiction = 'International';
  if (/\bBE\b/.test(input)) jurisdiction = 'Belgium (BE)';
  else if (/\bNL\b/.test(input)) jurisdiction = 'Netherlands (NL)';
  else if (/\bDE\b/.test(input)) jurisdiction = 'Germany (DE)';
  else if (/\bPT\b/.test(input)) jurisdiction = 'Portugal (PT)';
...
export const commandMap: Record<string, (args?: string, ctx?: any) => NexaCommandResponse> = {
...
export const findCommand = (cmd: string) => commandMap[cmd] || null;

export const handleSlashCommand = (cmd: string, args?: string, ctx?: any): NexaCommandResponse => {
